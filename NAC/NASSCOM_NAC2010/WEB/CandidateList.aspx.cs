...
namespace NASSCOM_NAC.Web
{
	...
	public partial class CandidateList : System.Web.UI.Page
	{
		...
		private void GetCandidateList(BLSearch objBLSearch)
		{
			...
			dsSearchCandidate = (DataSet) objBLSearch.SearchCandidate();
		...
		protected void btnViewJobCard_Click(object sender, System.EventArgs e)
		{
			...
			{
				Session["SortExp"] = lblSortExp.Text;
				Response.Redirect("MultipuleJobFairCard.aspx?SearchType=full");
			}
			else
			{
				bool IsSelected = false; 
				...
				if(IsSelected)
				{
					...
					Session["SortExp"] = lblSortExp.Text;
					Session["ItemList"] = strItemList.ToString().TrimEnd(',');
					Response.Redirect("MultipuleJobFairCard.aspx");
				}
				else
				{
					lblSortExp.Text = strSortExp;
