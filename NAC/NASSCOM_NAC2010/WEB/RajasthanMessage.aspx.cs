...
{
    /// <summary>
    /// Summary description for RajasthanMessage.
    /// </summary>
    public partial class RajasthanMessage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                try
                {
                    ...
                    //NAC Release 5.0
                    string strRegistrationID = Convert.ToString(dtUserDetail.Rows[0]["RegistrationID"]);
                    //
                    ...
                    if (dtUserDetail != null && Session["MailSent"] == null)
                    {
                        if (dtUserDetail.Rows.Count > 0)
                        {
                            ...
                            {
                                //NAC Release 5.0
                                SendMailWithPDF(Convert.ToString(dtUserDetail.Rows[0]["EmailId"]), Convert.ToString(dtUserDetail.Rows[0]["FullName"]), Convert.ToString(dtUserDetail.Rows[0]["PhotoIdDocument"]), Convert.ToString(dtUserDetail.Rows[0]["PhotoIdNo"]), Convert.ToString(dtUserDetail.Rows[0]["Password"]), strRegistrationID);
                                //
        ...
        private void SendMailWithPDF(string strEmailId, string strCandidateName, string strPhotoIDDocument, string strPhotoIDDocumentNumber, string strPassword, string strRegistrationID)
        {
            ...
            try
            {
                ...
                EmailBody += "</tr>";
                EmailBody += "<tr>";
                EmailBody += "<td colspan=3><p><span style=FONT-SIZE:9.0pt;FONT-FAMILY:Arial><strong>Please note</strong> - Your Admission Card will be made available on the website from  <font color=#6633ff>15-Sep-08</font> onwards. Please visit the website accordingly to download your Admission Card. </span></p></td>";
                EmailBody += "</tr>";
                EmailBody += "<tr>";
